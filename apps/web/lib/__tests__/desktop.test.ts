...
describe("getDesktopBridge", () => {
  ...
  test("returns the preload bridge when exposed", () => {
    ...
    const bridge: Poc2DesktopBridge = {
      ...
      hyprOverlayPreparePriceIcons: async () => ({}),
      rewardWatcher: async (enabled) => enabled,
      rewardWatcherStatus: async () => false,
      clipboardWrite: async () => true,
      marketHistoryAdd: async (entry) => ({
        id: "h1",
        createdAt: "2026-01-01T00:00:00.000Z",
        ...entry,
      }),
      marketHistoryList: async () => [],
      scanDiagnostics: async () => null,
      scanDiagnosticsSet: async (diagnostics) => diagnostics,
      pricesSnapshot: async () => ({ league: "L", names: [], byName: {}, unitIcons: {}, fetchedAt: null }),
      pricesStatus: async () => ({
        league: "L",
        count: 0,
        fetchedAt: null,
