...
- UI flows and React components: `docs/41-ui-flows.md`, `apps/web/components/` (one panel per workflow: `ItemEditor`, `TargetEditor`, `GuidePanel`, `EligibleTab`, `HistoryTab`, `DatabasePanel`, `PricePanel`, `GenesisPanel`, `ToolsPanel`, `SettingsPanel`, `OutcomeDialog`), the `Console` shell, and the Zustand store `apps/web/lib/store.ts`.
