...
- Work integrates on **`dev`** and ships from **`master`** (see Branch & Release Flow). Public remote: `grok-insider/poc2`; latest public tag **v2.0.0**.
...
- **Known gaps (the honest list — also mirrored in the roadmap):**
  - Trained models are **solved on demand** (ADR-0015): `recommend` analytically solves any cache-missing `(goal, item-class)` inside the engine worker (`training::solve`, sub-second) and caches the exact policy — the optional `/trained-models.json` corpus artefact is only a warm start (operator asset, never committed; regenerate in ~30 s with `train-advisor`). The ⚛ topbar chip reflects live cache growth; league/plugin-dispatch changes clear the cache.
  - Plugins: **phases 1+2 wired** (ADR-0014 browser-side JS host — Settings → Plugins; strategy/rule emission via `Engine.setPluginContent`; live custom predicates via `Engine.setPluginDispatch`, worker-held instances, strike-based auto-disable). **Phase 3 pending**: recommendation emitters need a planner-side candidate hook. The v1 SDK arena bug (absolute addresses as Vec indices — every real emission trapped) was found and fixed in this pass; `pluginsRealWasm.test.ts` pins the real SDK-built example plugin.
  ...
  - Data gaps CLOSED this pass: Waystone (class `Map`) / Precursor Tablet (`TowerAugmentation`) / Relic / Life+Mana Flask / Charm (`UtilityFlask`) / Inscribed Ultimatum mod pools ship in the bundle (near-exact poe2db parity; isolation pinned per surface — `pipeline/tests/live_bundle_pools.rs`). Still open: Thrud's Might, Preserved Vertebrae (bone→waystone path), Breach Ring quality caps, Abyss essence dual-mod, Vaal Catalysing Infuser, Expedition Saga omens, Logbooks.
...
- **App wiring (ADR-0014)**: browser-side JS host (`apps/web/lib/plugins/` — sandboxed `WebAssembly.instantiate`, no imports). Phase 1: emission exports → `Engine.setPluginContent` (set semantics). Phase 2: instances live in the engine worker (`__loadPlugins` transfer message); `Engine.setPluginDispatch` installs the sync predicate callback, strike-guarded (`lib/plugins/dispatch.ts`). Phase 3 (emitters) pending — needs a planner candidate hook.
...
- Train advisor models (full corpus, ~seconds): `cargo run --release --bin train-advisor -- --corpus pipeline/data/training_goals.toml --bundle ~/.config/poc2/bundles/poc2.bundle.json.gz --out ~/.config/poc2/cache/trained_models/poc2-trained-models-0.5.0.json --verbose`. The default `--model analytic` builds **exact** transition distributions from the engine's pool-weight enumeration (`enumerate_eligible_mods`), so there is no sampling budget to tune — `--samples` only sizes the MC fallback for actions without a closed form. The training patch follows the bundle's header.
- The historical Monte Carlo learner survives as `--model mc` (cross-validation reference; `--samples 100000` ≈ hours). The analytic-vs-MC agreement is pinned by `analytic_cross_validates_against_monte_carlo_per_item` in `crates/advisor/src/training/analytic_model.rs`. **Always pass `--bundle`** — without it the binary trains against an empty synthetic registry and every goal's `V_path(s0)` degenerates to the value-iteration floor (`-1000`). Add `--strict-audit` in CI to fail-fast when corpus goals reference concepts the bundle's mod taxonomy doesn't carry.
